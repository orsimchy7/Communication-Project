...
% 1 = Generate Chirp (Save Tx .mat file)
% 2 = Decode Chirp (Load recorded_chirp.mat & process)
% 3 = Simulation (Pass through artificial channel & process)
% ==========================================
RUN_MODE = 3; 
...
t = (1/Fs:1/Fs: T)';
f0 = 0.05*Fs;       % start frequency (10 kHz)
f1 = 0.4*Fs;        % end frequency (80 kHz)
...
if RUN_MODE == 2
    ...
    if ~isfile('recorded_chirp.mat')
        error('recorded_chirp.mat not found in current directory.');
    end
    rx_data = load('recorded_chirp.mat');
    
    % Dynamically grab the first variable inside recorded_chirp.mat 
    fields = fieldnames(rx_data);
    rx_signal = rx_data.(fields{1}); 
    rx_signal = rx_signal(:); % Ensure it's a column vector
    disp(['Loaded received signal from variable: ', fields{1}]);
...
% Transpose h_discrete to a row vector to find roots correctly
zplane(h_discrete', 1); 
>>>>>>> 4d1a4187b99aecc44e4fdc8db830631e176de884
