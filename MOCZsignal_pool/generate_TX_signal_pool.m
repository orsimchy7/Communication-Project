...
baseDir = fullfile(pwd, 'TX_signals');
dirK   = fullfile(baseDir, 'K_changes');
dirL   = fullfile(baseDir, 'L_changes');
dirSNR = fullfile(baseDir, 'SNR_changes');
...
for k_val = K_vec
    for iter = 1:num_signals_per_type
        ...
        fileName = sprintf('signal_K%d_L%d_Amp100_run%d.mat', k_val, baseParams.L, iter);
...
L_vec = 5:5:50; % Guard time in symbols (which directly equals ms since Tsym = 1ms)
for l_val = L_vec
    for iter = 1:num_signals_per_type
        simParams = baseParams;
        simParams.L = l_val; 
        ...
        % Save with iteration number
        fileName = sprintf('signal_K%d_L%d_Amp100_run%d.mat', baseParams.K, l_val, iter);
...
for p_scale = power_scales
    for iter = 1:num_signals_per_type
        ...
        % Save with iteration number
        fileName = sprintf('signal_K%d_L%d_Power%d_run%d.mat', baseParams.K, baseParams.L, p_label, iter);
