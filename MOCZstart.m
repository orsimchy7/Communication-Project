...
% -- Params --
B = 1; %messages num
K = 8; %bits per message
Tsym = 1e-3;
fc = 15e3;
Fs = 200e3;
lambda = 1; %according to articles
Kidxs = 1 : K;
figFlag = 0;
SNR = 5; %[dB]
BW = 1 / Tsym; %assuming beta = 1
...
x_mod = filter(h_coeff, normalization_factor, x_upsamp);
...
x_pb = real(x_mod.*exp(1i*2*pi*fc*t));

x_pb_noisy = awgn(x_pb, SNR ,'measured');
...
mixed_x = x_pb_noisy.*exp(-1i*2*pi*fc*t);
...
figure; plot(1:9, [abs(x_decoded), abs(x)]);
...
% figure
% scatter(real(decoded_alphas), imag(decoded_alphas), 50, 'filled', 'b');
% grid on;
% axis equal;
% hold on;
% scatter(real(alphas), imag(alphas), 50, 'filled', 'g');
% hold on;
% thetas_for_plot = linspace(0, 2*pi, 300);
% out_circle_points = R*exp(1i*thetas_for_plot);
% in_circle_points = (1/R)*exp(1i*thetas_for_plot);
% plot(out_circle_points, 'r', 'LineWidth',1 )
% hold on;
% plot(in_circle_points, 'r', 'LineWidth',1)
% legend(['decoded zeros','encoded zeros', '', '']);
% hold off;
